module Combinators where
    ...
    expression :: [(Assoc, [(Parser str b, a -> a -> a)])] ->
                  Parser str a ->
                  Parser str a
    expression ops primary = foldr (\(associativity, operators) acc -> step associativity operators acc) primary ops
      where
        step:: Assoc -> [(Parser str b, a -> a -> a)] -> Parser str a -> Parser str a
        step associativity = case associativity of
            LAssoc -> leftStep
            RAssoc -> rightStep
            NAssoc -> nStep

        leftStep :: [(Parser str b, a -> a -> a)] -> Parser str a -> Parser str a
        leftStep operators primary = do
            left <- primary
            ...
            return $ foldl (\acc elem -> elem acc) left right

        rightStep :: [(Parser str b, a -> a -> a)] -> Parser str a -> Parser str a
        rightStep l primary = do
            left <- primary
            let l' = fmap (\(parser, op) -> parser *> pure op) l
            let l'' = foldr1 (\parser acc -> parser <|> acc) l'
            op <- l''
            right <- rightStep l primary
            return $ op left right
            <|> primary

        nStep :: [(Parser str b, a -> a -> a)] -> Parser str a -> Parser str a
        nStep l primary = do
            left <- primary
            let l' = fmap (\(parser, op) -> parser *> pure op) l
            let l'' = foldr1 (\parser acc -> parser <|> acc) l'
